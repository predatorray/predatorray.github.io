...
在面向对象设计原则中，有一个让人最难理解的是“命令而不要去询问”原则。这个原则鼓励去命令一个对象做一个事情，而不是把这个对象的数据都取出来自己做。mockist测试者认为使用mock风格的测试能够促进遵守这个原则并且避免现在常常被滥用的一坨getter代码。而经典测试风格的人认为可以用其他的方式来做到。
